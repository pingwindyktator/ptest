...
namespace ptest {
    ...
    static struct {
        ...
        void set_max_time (const std::chrono::duration<Rep, Period> &max) {
          if (max > std::chrono::milliseconds::max() - std::chrono::milliseconds(15)) {
            throw std::runtime_error("invalid max_time set");
          }
          max_time = max;
    ...
    void _print_thread_safe (std::ostream &out, const First &value) {
      std::mutex mtx;
    ...
    void _print_thread_safe (std::ostream &out, const First &value, const Rest &... rest) {
      std::mutex mtx;
      std::lock_guard<std::mutex> lock(mtx);
      out << value;
    ...
    template <typename func_t, typename ... Args>
    auto _run_function_with_timeout (func_t func, const Args ... args) -> function_status<decltype(func(args...))> {
      auto start = std::chrono::high_resolution_clock::now();
    ...
    template <typename func_t, typename ... Args>
    auto _run_function_without_timeout (func_t func, const Args &... args) -> function_status<decltype(func(args...))> {
      auto start = std::chrono::high_resolution_clock::now();
    ...
    template <typename func_t, typename ... Args>
    auto _run_function (bool timeout, func_t func, const Args ... args) -> function_status<decltype(func(args...))> {
      return (timeout ? _run_function_with_timeout(func, args...) : _run_function_without_timeout(func, args...));
    }
    ...
    void _run_test (func_t function,
      ...
      try {
        result = _run_function(configuration.is_timeout_active, function, args...);
      } catch (...) {
        ++stats.timeout;
