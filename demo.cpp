...
int main () {
  ...
  int aa = 99;
  ptest::general.local_config.print_passed_tests = false;
  ...
  ptest::ptest_suite suite1("suite1");
  suite1.local_config.print_passed_tests = true;

  run_suite_test(suite1, func2, 5, vec, 5, 0);
  run_suite_test(suite1, func, 1, 1, "ehe xd\"", 3.14, '\'', 2, 0);
  run_suite_test(suite1, func3, "ala ee", "ala a", "ala b", 'a', 'b', "al\"}", "al\"}", "al\"}", '\"', '\"', 44, 1);
  suite1.print_suite_result();
  print_final_result();
  return 0;
}